...
#undef toggle_loop
#define toggle_loop( toggleCondition )  \
do { \
while (1)   \
{ \
...
int main(int argc, char *argv[])
{
    ...
    int opt;
    int directionFlag = 0, toggleValue = 64, channel = 0, param = 0, argsFlag = 0;
    while((opt = getopt(argc, argv, "c:p:t:r")) != -1)
    {  
        switch(opt)
        {  
    ...
    if( directionFlag )
    {
        toggle_loop( inputEvent->data.control.value >= toggleValue && previousValue < toggleValue );
    }
    else
    {
        toggle_loop( inputEvent->data.control.value <= toggleValue && previousValue > toggleValue );
